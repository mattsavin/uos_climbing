...
const PAGES = [
    ...
    },
    {
        file: 'gear.html',
        path: '/gear',
        title: 'Gear Hire | USMC',
        description: 'Browse the USMC gear store — harnesses, helmets and belay devices available for members to hire.',
        memberOnly: true
    },
    {
        file: 'login.html',
...
function buildManagedBlock(page, faqs) {
    ...
    if (page.memberOnly) {
        ...
        // faqs is pre-extracted and validated non-empty at the top of the script.
...
// Extract once — and fail the run (non-zero exit) if the config parse turns up
// nothing: silent exit-0 here would ship pages with missing/stale FAQ schema
// and nobody would notice until a consumer queries it. Wired into `prebuild`
// so CI catches drift too.
const faqs = extractFaqs();
if (faqs.length === 0) {
    console.error(
        'FATAL: extractFaqs found no Q&A pairs in src/config.ts — refusing to generate pages without FAQ schema. Check that the faqs array and its closing bracket are still in the expected shape.'
    );
    process.exit(1);
}
...
for (const page of PAGES) {
    injectSeo(path.resolve('src', '..', page.file), buildManagedBlock(page, faqs));
}
